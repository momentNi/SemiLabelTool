...
class SegmentAnythingONNX:
    ...
    @staticmethod
    def get_preprocess_shape(old_height: int, old_width: int, long_side_length: int):
        """
        Compute the output size given input size and target long side length.
        """
        scale = long_side_length * 1.0 / max(old_height, old_width)
    ...
    def run_decoder(self, image_embedding, original_size, transform_matrix, prompt):
        ...
        transformed_masks = self.transform_masks(masks, original_size, inv_transform_matrix)
